...
画像生成は Codex app-server + `$imagegen` を worker 経由で実行する前提です。
Clerk と Stripe の環境変数を設定すると外部AuthとCheckoutに接続し、未設定のローカル開発ではデモモードで動作します。
MVP ではローカル生成アセット、ダミーデータ、ローカル台帳で以下の制作パイプラインを UI として操作できます。
...
ローカル開発だけなら `.env` なしで起動できます。本番に近づける場合は以下を設定します。
...
STRIPE_PRICE_STANDARD_24=price_...
STRIPE_PRICE_CREATOR_LITE=price_...
STRIPE_PRICE_CREATOR_PRO=price_...
STRIPE_PRICE_STUDIO=price_...
STRIPE_PRICE_EXTRA_10=price_...
...
Clerkキーがある場合は `/app` と主要APIがClerkで保護されます。未設定の場合はローカル開発用のデモユーザーで動きます。
Stripeキーがある場合は Checkout Sessions を作成します。未設定の場合は `/app/billing?checkout=demo` へ戻すデモCheckoutになります。
...
- `/login`
  - Clerk設定時はClerk SignIn、未設定時は開発デモログインUI
- `/register`
  - Clerk設定時はClerk SignUp、未設定時は利用規約・プライバシーポリシー同意付きの開発デモ登録UI
- `/demo`
...
- `/app`
  - アプリ内ダッシュボード
- `/app/projects`
  - キャラクターシート一覧、検索、ステータスフィルタ
- `/app/templates`
  - アプリ内テンプレート適用UI
- `/app/settings`
  - スタジオ設定、AI生成表記、法務リンク、利用量、Auth/Stripe/Storage接続状況
- `/app/billing`
  - 単発売り・月額プランの利用量確認UI、Stripe Checkout開始ボタン
- `/app/projects/demo`
  - 制作ダッシュボード
  - スタンプ数切り替え、文字モード切り替え、セリフテンプレート、Creators Market 自動チェック、AI生成コンテンツ注意、PNG入りZIP書き出し、申請パックURL発行
...
QA_BASE_URL=http://localhost:3002 npm run qa
```
...
- 登録、ログイン、デモアカウント導線
- アプリ内ホーム、キャラクターシート一覧、テンプレート、設定、プラン画面
...
OpenRouter または OpenAI Images API は将来差し替え用stubです。現時点の本命は Codex app-server + `$imagegen` です。別プロバイダーへ差し替える場合は、以下の環境変数名を想定しています。

- `OPENROUTER_API_KEY`
- `OPENROUTER_IMAGE_MODEL`
- `OPENROUTER_BASE_URL`

ローカル開発では `.env` への依存はありません。
...
`POST /api/generation-jobs` は利用量台帳から生成クレジットを消費します。
...
- `GET /api/submission-packs/:token/zip`
  - `submission-manifest.json`、メイン画像、タブ画像、スタンプPNGを含む申請パックZIPを返却
...
`POST /api/submission-packs` は利用量台帳から書き出し数を1消費します。`GET /api/submission-packs/:token` とZIP取得はChrome拡張が短期トークンだけで読めるよう、Clerkログイン不要のURLとして扱います。

## Auth・決済・利用量

- Auth: Clerkを採用します。`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` と `CLERK_SECRET_KEY` が設定されると、middlewareで `/app` と主要APIを保護します。
- 決済: Stripe Checkout Sessionsを使います。単発売りは `mode=payment`、月額プランは `mode=subscription` です。
- Webhook: `POST /api/stripe/webhook` で `checkout.session.completed` を受け、購入プランに応じて `.data/usage-ledger.json` の利用量上限を更新します。追加生成オプションは現在のプランを維持したまま上限へ加算します。
- 利用量: `GET /api/usage` で現在のユーザー、生成クレジット、書き出し数、接続ステータスを返します。
- ストレージ: 既定は `public/generated` のローカル配信です。`ASSET_STORAGE_DRIVER=r2` とR2環境変数を設定すると、R2接続準備状態を表示できます。
...
- 本番運用ではClerkによる外部Authを使います。Clerk未設定のローカル開発時のみ、ブラウザの `localStorage` を使ったデモセッションにフォールバックします。
- 利用量台帳とジョブストアは初期MVPでは `.data/*.json` です。本番ではDB、オブジェクトストレージ、監査ログ、バックアップ、データ削除手段へ置き換えてください。
